...
import { FormatTooltip, SourceTag, InfoTooltip, IRemoteTarget } from "..";

export interface PEntityMetaToolTip extends IRemoteTarget {
  field: any;
}
...
export function EntityMetaToolTip(props: PEntityMetaToolTip) {
        ...
        if (attribute) {
          const atts = {
            ...
            Source: attribute.source ? (
              <SourceTag
                source={attribute.source}
                className="tol-entity-meta-tool-tip-source"
              />
            ) : (
              <></>
            ),
  ...
  return <InfoTooltip contents={Tooltip} />;
}
